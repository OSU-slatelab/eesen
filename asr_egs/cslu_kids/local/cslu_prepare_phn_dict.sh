...
[ -f $dir/cmudict ] && svn co -r 11745 https://svn.code.sf.net/p/cmusphinx/code/trunk/cmudict $dir/cmudict
...
cat $dir/lexicon2_raw_nosil.txt | \
  sed s/[012]//g \
  > $dir/lexicon3_raw_nosil.txt

# Find OOV words and add them via g2p
#python local/cslu_find_oov.py --lexicon $dir/lexicon3_raw_nosil.txt --text data/train/text --output $dir/OOVs.txt
#python local/cslu_find_oov.py --lexicon $dir/lexicon3_raw_nosil.txt --text data/dev/text --output $dir/OOVs.txt
#python local/cslu_find_oov.py --lexicon $dir/lexicon3_raw_nosil.txt --text data/test/text --output $dir/OOVs.txt

#g2p-seq2seq --decode $dir/OOVs.txt --model_dir /u/plantinga.1/Documents/Repositories/g2p-seq2seq/g2p-seq2seq-model-6.2-cmudict-nostress/ --output $dir/OOVs_pronunciation.txt

cat $dir/OOVs_pronunciation.txt | tr '[:lower:]' '[:upper:]' >> $dir/lexicon3_raw_nosil.txt

# Get the set of lexicon units without noises
cut -d' ' -f2- $dir/lexicon3_raw_nosil.txt | tr ' ' '\n' | sort -u > $dir/units_nosil.txt
...
#(echo '<SIL> <SIL>'; ) | \
# cat - $dir/lexicon3_raw_nosil.txt | sort | uniq > $dir/lexicon.txt || exit 1;
cat $dir/lexicon3_raw_nosil.txt | sort | uniq > $dir/lexicon.txt || exit 1;
...
#(echo '<SIL>';) | cat - $dir/units_nosil.txt | awk '{print $1 " " NR}' > $dir/units.txt
