...
with open(config["phn_lexicon"],"r") as f, open(config["output_units"],"w") as output_units:
    ...
    for line in f:
        ...
        if config["ignore_noises"] and \
                (("[" in  line.split()[0] and "]" in line.split()[0]) or \
                    ("<" in  line.split()[0] and ">" in line.split()[0])):
            print("ignoring " + line.split()[0])
            continue
    ...
    for text_line in input_text:
        ...
        for word in text_line.split()[1:]:
            if word in lexicon_phn:
                #new_line = new_line + " " + str(dict_phn["<SIL>"]) + lexicon_phn[word]
                new_line += lexicon_phn[word]
            elif word.lower() in lexicon_phn:
                new_line += lexicon_phn[word.lower()]

        #output_labels.write(new_line + " " + str(dict_phn["<SIL>"]) + "\n")
